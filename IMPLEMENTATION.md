...
| **Forgetting / pruning** | The agent supersedes stale syntheses via `retain {…, "supersedes":[ids]}` (joyvend deletes them after inserting the replacement); **orphan entities are pruned** automatically on delete (+ `store.PruneOrphans`). Mirrors hindsight's graph-maintenance / fold-and-delete — but LLM-adjudicated semantic dedup + auto-consolidation stay the agent's job (they need an internal LLM, which joyvend has by design). |
| **Auto-retain (capture + distill)** | Fixes *silent under-retention* without adding reasoning to joyvend: `POST /…/capture` + `joyvend capture` log each raw turn as a low-tier `experience` tagged `capture`, with **mechanical (no-LLM) dedup** (cosine ≥0.97 via `EmbedQuery`+`VectorSearch`). Captures are **excluded from recall/reflect by default** (`include_captures` opts in; `?type=&tag=` lists them). A host hook makes the *trigger* automatic; the agent **distills** captures → `mental_model` via the existing `retain{supersedes}` (folds + deletes raw rows). Recipes in `integrations/claude-code/` (`UserPromptSubmit` capture + `Stop` distill nudge). Judgment stays the agent's. |
| **Agent guide** | `GET /v1/guide` + `joyvend guide` + a GUI "Copy full instructions" button serve the host LLM's **operating manual** (the full retain/recall/reflect/supersede protocol + memory types) — joyvend's equivalent of hindsight's ~40 KB reflect-agent system prompt. Required because the agent does all reasoning. |
